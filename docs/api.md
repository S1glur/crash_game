...
### `PUT /api/config` — НЕ РЕАЛИЗОВАН
Задуман для админки: валидация значений, запись в `config/game.json`, применение
без рестарта. Пока настройки меняются только правкой файла — обязательный
минимум ТЗ («конфигурационный файл с горячей перезагрузкой») этим закрыт,
веб-админка относится к дополнительным возможностям.
